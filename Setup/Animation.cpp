...
Animation::Animation(SDL_Texture* spriteSheet, SDL_Renderer* renderer, int numberOfFrames,
...
void Animation::update(float dt)
{
	...
	if (frameTimer >= frameDuration)
	{
		...
		{
			//if cycle through all frames, go back to the beginning (reset animation)
			currentFrame = 0;
		}
	}
}
...
void Animation::draw(int x, int y, bool flip)
{
	...
	//get the correct flip flag to help mirror out image if we want to
